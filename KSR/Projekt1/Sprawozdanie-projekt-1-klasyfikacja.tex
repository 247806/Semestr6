...
Na podstawie wyników można zaobserwować, że miara Accuracy rośnie wraz ze wzrostem parametru \(k\) do wartości \(k = 5\), osiągając maksimum równe 0{,}8904. Dalsze zwiększanie wartości \(k\) prowadzi jednak do spadku dokładności klasyfikatora. Może to świadczyć o nadmiernym uogólnieniu modelu przy zbyt dużej liczbie sąsiadów, co skutkuje mniejszą precyzją klasyfikacji.

Optymalną wartością parametru \(k\) w tej konfiguracji wydaje się być \(k = 5\), zapewniającą najlepszy kompromis między stabilnością a dokładnością decyzji klasyfikatora.
...
\begin{table}[h!]
    ...
    1 & Metryka euklidesowa & 0.8864\\ \hline
