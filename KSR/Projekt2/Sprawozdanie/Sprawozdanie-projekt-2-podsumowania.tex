...
\begin{itemize}
    \item[-] prawie żaden
        ...
        \begin{cases}
        \exp\left( -\frac{1}{2} \left( \frac{x - 0{,}0}{\frac{0{,}2 - 0{,}0}{3}} \right)^2 \right), & x \in (0, 0.2) \\
        0, & \text{w przeciwnym razie}
        \end{cases}
        \end{equation}
    \item[-] trochę
        \begin{equation}
            \mu_{\text{trochę}}(x) =
        \begin{cases}
       \exp\left( -\frac{1}{2} \left( \frac{x - 0{,}3}{\frac{0{,}3 - 0{,}15}{3}} \right)^2 \right), &  x \in (0.15, 0.45) \\
        0, & \text{w przeciwnym razie}
\end{cases}
        \end{equation}
    \item[-] około połowa
        ...
        \begin{cases}
        \exp\left( -\frac{1}{2} \left( \frac{x - 0{,}5}{\frac{0{,}5 - 0{,}3}{3}} \right)^2 \right), & x \in (0.35, 0.75) \\
        0, & \text{w przeciwnym razie}
\end{cases}
        \end{equation}
    \item[-] wiele
        \begin{equation}
            \mu_{\text{wiele}}(x) =
            \begin{cases}
            \exp\left( -\frac{1}{2} \left( \frac{x - 0{,}75}{\frac{0{,}75 - 0{,}60}{3}} \right)^2 \right), & x \in (0.60, 0.85) \\
            0, & \text{w przeciwnym razie}
            \end{cases}
        \end{equation}
    \item[-] prawie wszystkie
        \begin{equation}
            ...
            \begin{cases}
            \exp\left( -\frac{1}{2} \left( \frac{x - 1{,}0}{\frac{1{,}0 - 0{,}8}{3}} \right)^2 \right), &  x \in (0.8, 1.0) \\
            0, & \text{w przeciwnym razie}
            \end{cases}
        \end{equation}
\end{itemize}
...
Program został przygotowany w języku Java w wersji JDK 24, dodatkowo do zarządzania relacyjną bazą danych - PostgreSQL został wykorzystany framework Spring Boot. Kod źródłowy pakietu obliczeń rozmytych o nazwie \(ligustic\) został podzielony na dwa dodatkowe pakiety. Pierwszy z nich - \(set\), zawiera podstawowy model reprezentujący zbiory rozmyte. Centralnym elementem tego pakietu jest klasa abstakcyjna FuzzySet, która definiuje fundamentalne operacje na zbiorach rozmytych, są to m.in. funkcje, które zwracają kardynalność lub nośnik zbioru rozmytego. Klasy: GaussianFunction, TrapezoidalFunction oraz TriangularFunction są rozszerzeniami klasy FuzzySet, które implementują przynależność do zbioru rozmytego w zależności od wybranej funkcji (gaussowka, trapezoidalna lub trójkątna). \\
Pakiet \(lingustic\) składa się z klas, które są odpowiedzialne za tworzenie podsumowań lingwistycznych. Możemy wyodrębnić LingusticVariable, - klasa reprezentująca zmienną lingwistyczną, która charakteryzuje określoną cechę (np. temperatura, wilgotność). Zmienna zawiera nazwę, przestrzeń rozważań oraz zestaw terminów lingwistycznych (np. "bardzo zimno", "ciepło").
Kwalifikator oraz sumaryzator mają bardzo podobne struktry, różnią się tylko celami użycia, w jaki sposób są wykorzystywane w podsumowaniach lingwistycznych, dlatego została stworzona wspólna klasa LingusticTerm, składająca się z nazwy i zbioru rozmytego, z której dziedziczą klasy Qualifier oraz Summarizer w celu zachowania semantycznego rozróżnienia. Klasa Quantifier opisuje kwantyfikator poprzez nazwę, zbiór rozmyty i czy to jest kwantyfikator absolutny czy względny. Najważniejszą klasą w pakiecie jest Summary, która odpowiada za generowanie podsumowań lingwistycznych w oparciu o dostarczone sumaryzatory, kwalifikator oraz kwantyfikator. \\

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/lingustic.png}
\caption{Diagram UML pakietu \(lingustic\).}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{img/set.png}
